...
int main( void )
{
  ...
  //初始化板载的相关设备，如时钟振荡器，缓存预取控制、LED
  //LED部分的代码需要修改，端口设置不一样。
  //也就是hal_borad_cfg.h这个文件很多都要更改
  HAL_BOARD_INIT(); //Y
  ...
  //这个地方没什么用，参数为OB_COLD就是去检测reset的原因。
  //如果确实是reset，如电源、外部、看门狗或者未知的原因，执行相应的处理函数
  //提高稳定性倒是挺有用的，然而目前没什么用
  //不用修改
  InitBoard( OB_COLD ); //X

  // Initialze HAL drivers
  //初始化硬件，没有理解为什么LED的初始化在上面有了，这里还要初始化一边LED。
  //并且，为什么要时钟,LED初始化，和这个分开呢？
  //当然，上面的是初始化端口，这里的是初始化行为--灭或者闪烁，但没必要分开呀。
  //还有这里设置了KEY的SEL和DIR，而其他关于KEY的参数设置又放到了下面，这也太理解。
  //难道是因为怕用户误操作，所以等所有设置完了再进行KEY的完全初始化？
  //要修改。
  HalDriverInit(); //Y

  // Initialize NV System
  //初始化非易失寄存器，也就是对FLASE的初始化，无需修改
  osal_nv_init( NULL ); //X
  ...
  //不清楚初始化了哪些全局变量，也不清楚这些变量有什么用。
  //无需修改
  zgInit(); //X

#ifndef NONWK
  // Since the AF isn't a task, call it's initialization routine
  //这个就是如果禁止了NWK，APS，ZDO的函数功能，要单独初始化AF层
  //没有禁用，不用管
  afInit(); //X
#endif
  ...
  //此处参数是OB_READY,就是对KEY进行配置，这里是配置，而上面是初始化KEY的端口。
  //好像有点道理，不过还是觉得是怕用户没在初始化完成前就操作发生错误，才放在这里进行配置的。
  //对KEY进行获取方式配置--扫描/中断，以及设置回调函数
  //需要修改
  InitBoard( OB_READY ); //Y

  // Display information about this device
  //显示设备的信息，需要定义LCD_SUPPORTED这个变量
  //没有定义，就相当于这个函数没有执行任何功能
  //无需修改，但要把option里的这个定义给删了
  zmain_dev_info(); //X

  /* Display the device info on the LCD */
#ifdef LCD_SUPPORTED
  //瞎起名字，好让人误解，这个并不是初始化LCD硬件，
  //是指初始化他显示的内容，好吧，仔细理解下还是可以理解的
  //不过你上一个函数刚显示信息，中间又没有延时，上个显示不就废了
  //比如LCD显示设备信息。是Coordinator还是end device
  //无需修改，没LCD。
  zmain_lcd_init(); //X
#endif
