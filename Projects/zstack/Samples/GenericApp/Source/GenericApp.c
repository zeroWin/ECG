...
//                                GENERICAPP_SEND_MSG_TIMEOUT );
  ...
  // Start Measure - This event is generated by a timer
  // 目前就发1-100，轮流发
  if ( events & GENERICAPP_START_MEASURE )
  {
    static uint8 num_send = 1;
  ...
  //Close Measure - This event is generated by a timer
  if ( events & GENERICAPP_END_MEASURE )
  {
    osal_stop_timerEx( GenericApp_TaskID , GENERICAPP_START_MEASURE );
    
    return (events ^ GENERICAPP_END_MEASURE);
  }
  ...
  if ( events & GENERICAPP_ECG_NETWORK_TEST )
  {
    ...
    else  //disconnect to GW
    {
      if ( EcgSystemStatus == ECG_ONLINE_MEASURE )
        HalEcgMeasStop();        // 关闭测量
...
void GenericApp_HandleKeys( byte shift, byte keys )
{
  if(keys & HAL_KEY_SW_6)   //Link button
  {
    if( EcgSystemStatus == ECG_OFFLINE_IDLE )
    {
    ...
    else if( EcgSystemStatus == ECG_ONLINE_IDLE)
    {
    ...
    else if ( EcgSystemStatus == ECG_FIND_NETWORK ) // 有问题
    {
      //Close Network test event
      osal_stop_timerEx( GenericApp_TaskID , GENERICAPP_ECG_NETWORK_TEST );     
